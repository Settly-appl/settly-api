...
**Settled vs declared** — `ExpenseSplit.settled` is the *owner's* word alone: the creditor confirming money actually arrived. A participant hitting the same settle endpoints does **not** settle — it sets `declaredPaid`/`declaredAt`, an "I paid" *claim* surfaced to the owner as a suggestion to verify and confirm (`ExpensePaymentDeclaredEvent` → owner notification). Declarations never affect balances, are cleared whenever a share becomes settled (owner confirm or settle-up), can be retracted by the participant, and exempt the debtor from the daily settlement reminder (`sumUnsettledByPair`). A participant cannot flip an owner-confirmed `settled` back. The endpoints stay the same (`PATCH .../settle`, `/unsettle`); who calls them decides the meaning (`ExpenseSplitService.setSplitSettled`).
...
**Balance drill-down** — `GET /balances/{counterpartyId}/expenses` returns the unsettled shares one balance is made of, both directions at once, each marked `owedToMe`. It reads the same unsettled splits `getBalances` sums (`findUnsettledBetweenWithExpense`, the fetch-joined twin of the settle-up query), so the list and the figure above it cannot disagree. Netting is deliberately left to the client: "you owe 40" and "you owe 90, you are owed 50" are the same balance, and only the second can be checked against the expenses behind it. A share whose expense has no rate carries `shareBaseAmount: null` and is listed anyway — it is missing from the balance, which is a thing to tell the user, not to hide. Scoped to the caller by construction: every row is a split where they are the payer or the debtor.
...
**Project date span** — `projects.start_date`/`end_date` are both nullable: most projects are not trips and have no dates. Only a project with **both** ends set takes part in the app's automatic project selection for a new expense — an open-ended range would claim every future expense. Order is enforced twice: `@AssertTrue` on the request records and a `CHECK` constraint on the table. On update the two move together (sending one alone would let a caller build an inverted span across two requests, which per-request validation cannot see). PATCH reads an absent field as "leave this alone", so **removing** a span needs its own flag, `clearDateSpan` — two absent dates cannot also mean "delete them", and a second `@AssertTrue` refuses a request that both clears and sets. A blank `defaultCurrency` plays the same role for the trip currency, and clearing it also clears `default_rate_to_base`: a rate belongs to a currency, and a leftover one converts nothing while still being there for a later edit to hand to a different currency.

**Settle-up reminder** — The daily job (`SettlementReminderJob`) nudges **net** debtors only. Holding an unsettled share is not the same as owing money: someone who owes Anna 50 while Anna owes them 200 has nothing to pay, because settling up moves money the other way — and that is what the job used to nag about, since it grouped unsettled shares by debtor alone. `sumUnsettledByPair` now returns both directions of every relationship and the job subtracts them, reminding only about counterparties it comes out behind with, for the net amount (the same figure the app's "Zobowiązania" shows). Netting is **per pair**: being owed by one person does not pay off another, and cross-netting would hide a debt nobody is going to cancel. A pair whose shares the debtor has all declared paid is skipped — the claim is precisely what buys them silence — but declared shares still count toward the balance, because a claim is not a payment.
